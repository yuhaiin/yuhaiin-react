...
function Tags() {
    ...
    const { data, error, isLoading, mutate } = useProtoSWR(tag, tag.method.list)
    const { data: nodes } = useProtoSWR(node, node.method.list)
    ...
    return (
        <>
            <ConfirmModal
                ...
                onOk={() => {
                    FetchProtobuf(tag, tag.method.remove, create(StringValueSchema, { value: confirmData.name }))
                        .then(async ({ error }) => {
            ...
            <TagModal
                ...
                onSave={() => {
                    ...
                    FetchProtobuf(tag, tag.method.save, tagModalData.tag)
                        .then(async ({ error }) => {
