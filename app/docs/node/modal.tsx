...
export const NodeModal: FC<{
    ...
    ({ hash, point, editable, show, onHide, onSave, groups, onDelete, isNew }) => {
        ...
        const { data: nodes, error, isLoading, isValidating, mutate } = useSWR(
            hash === "" ? undefined : ProtoPath(node, node.method.get),
            ProtoESFetcher(node, node.method.get, "POST", create(StringValueSchema, { value: hash }), point,),
            {
        ...
        const SaveButton = () => {
            ...
            return <Button
                ...
                onClick={() => {
                    if (!nodes) return
                    if (isNew) nodes.hash = ""
                    FetchProtobuf(node, node.method.save, nodes)
                        .then(async ({ error }) => {
                            if (!error) {
    ...
    const Footer = () => {
        ...
        return <Button variant="outline-primary"
            onClick={() => {
                ...
                FetchProtobuf(node, node.method.save, p)
                    .then(async ({ error }) => {
                        if (error === undefined) {
                            ctx.Info("save successful")
