...
import { hostSchema, inboundSchema, mode, modeSchema, or, orSchema, processSchema, resolve_strategySchema, rule, ruleSchema, rulev2Schema, udp_proxy_fqdn_strategy, udp_proxy_fqdn_strategySchema } from '../../pbes/config/bypass/bypass_pb';
...
const RuleRow: FC<{
    ...
    return (
        <InputGroup className="mb-3">
            <div className='form-control p-0' style={{ flex: '0 0 25%' }}>
                <Select
                    styles={{
            ...
            <Modal.Body>
                {error ?
                    ...
                    isValidating || isLoading || !rule ? <Loading /> :
                        <>
                            <SettingTypeSelect label='Mode' type={modeSchema} filter={(v) => v.number !== mode.bypass} value={rule.mode} onChange={(v) => setRule({ ...rule, mode: v }, false)} />
                            <SettingInputText label='Tag' value={rule.tag} onChange={(e) => setRule({ ...rule, tag: e }, false)} />
                            <SettingTypeSelect label='Resolve Strategy' type={resolve_strategySchema} value={rule.resolveStrategy} onChange={(e) => setRule({ ...rule, resolveStrategy: e }, false)} />
