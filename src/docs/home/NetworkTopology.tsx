...
import { ArrowRight, Map as MapIcon, Maximize2, Minimize2, Minus, Plus, RotateCcw, ShieldCheck } from "lucide-react";
...
function buildTopology(connections: Connection[]): TopologyGraph {
    ...
    const contentTop = 70;
    const contentBottom = height - 30;
    const contentCenter = (contentTop + contentBottom) / 2;
    const firstYByKind = new globalThis.Map<TopologyKind, number>();
    for (const { kind } of columnLabels) {
        const rowCount = rowsByKind.get(kind)?.length ?? 0;
        if (rowCount === 0) continue;
        const groupHeight = nodeHeight + (rowCount - 1) * 102;
        firstYByKind.set(kind, Math.max(contentTop, contentCenter - groupHeight / 2));
    }
    const positioned = rawNodes.map((node) => {
        const index = rowIndexById.get(node.id) ?? 0;
        const firstY = firstYByKind.get(node.kind) ?? contentCenter - nodeHeight / 2;
        const y = firstY + index * 102;
...
function nodePalette(kind: TopologyKind) {
    switch (kind) {
        case "device": return { fill: "var(--friendly-topology-device-fill)", stroke: "var(--friendly-topology-device-stroke)", accent: "var(--friendly-topology-device-accent)" };
        case "application": return { fill: "var(--friendly-topology-application-fill)", stroke: "var(--friendly-topology-application-stroke)", accent: "var(--friendly-topology-application-accent)" };
        case "inbound": return { fill: "var(--friendly-topology-inbound-fill)", stroke: "var(--friendly-topology-inbound-stroke)", accent: "var(--friendly-topology-inbound-accent)" };
        case "route": return { fill: "var(--friendly-topology-route-fill)", stroke: "var(--friendly-topology-route-stroke)", accent: "var(--friendly-topology-route-accent)" };
        case "outbound": return { fill: "var(--friendly-topology-outbound-fill)", stroke: "var(--friendly-topology-outbound-stroke)", accent: "var(--friendly-topology-outbound-accent)" };
    }
}
...
const TopologyNodeCard: FC<{ node: TopologyNode; selected: boolean; related: boolean; dimmed: boolean; onSelect: () => void }> = ({ node, selected, related, dimmed, onSelect }) => {
    ...
    return (
        <g data-topology-node="true" className={clsx("friendly-topology-node", related && "is-related", dimmed && "is-dimmed")} onPointerDown={(event) => { event.stopPropagation(); onSelect(); }} role="button" tabIndex={0} aria-label={`${node.label}, ${node.meta}`} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onSelect(); }}>
            ...
            <text x={node.x + 42} y={node.y + 27} fill="var(--friendly-topology-ink)" fontSize="14" fontWeight="700">{shorten(node.label, labelLength)}</text>
            <text x={node.x + 42} y={node.y + 49} fill="var(--friendly-topology-muted)" fontSize="11">{shorten(node.meta, metaLength)}</text>
...
const NetworkTopology: FC<{
    ...
    useEffect(() => {
        ...
        // Trackpad pinch is reported as a modified wheel event. Use a native
        // non-passive listener so the browser cannot turn it into page zoom.
        ...
        svg.addEventListener("wheel", handleModifiedWheel, { capture: true, passive: false });
        return () => svg.removeEventListener("wheel", handleModifiedWheel, true);
    }, [expanded, connections.length, zoomAtPoint]);
                    ...
                    >
                        <defs>
                            <marker id="friendly-topology-arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto" markerUnits="strokeWidth">
                                <path d="M0,0 L8,4 L0,8 z" fill="var(--friendly-topology-arrow)" />
                            </marker>
                        </defs>
                        <g transform={`translate(${viewport.x} ${viewport.y}) scale(${viewport.scale})`}>
                            {columnLabels.map((column) => <text key={column.kind} x={columnX[column.kind]} y="28" fill="var(--friendly-topology-muted)" fontSize="11" fontWeight="700">{column.label}</text>)}
                            {graph.edges.map((edge) => {
                                ...
                                return (
                                    <g key={edge.id} className={clsx("friendly-topology-edge", selectedPaths && (selectedPaths.edgeIds.has(edge.id) ? "is-highlighted" : "is-dimmed"))}>
                                        <path d={`M ${x1} ${y1} C ${x1 + bend} ${y1}, ${x2 - bend} ${y2}, ${x2} ${y2}`} fill="none" stroke="var(--friendly-topology-edge)" strokeWidth="2" markerEnd="url(#friendly-topology-arrow)" />
                                        {edge.count > 1 && <text x={(x1 + x2) / 2} y={(y1 + y2) / 2 - 5} fill="var(--friendly-topology-muted)" fontSize="10" fontWeight="700">×{edge.count}</text>}
                                    </g>
                                );
                            })}
